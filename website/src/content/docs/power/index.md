---
title: Intermediate
description: Power-user features of the CDC Badge OS - plugins, the serial console, the expert menu, languages, on-device storage tools and the companion scripts.
sidebar:
  order: 0
---

The pages in this section go past everyday use and into the controls a power
user reaches for: the WASM plugin runtime, the serial console, the Expert menu,
the language system, the on-device file and NVS tools, and the Python helpers in
`tools/`. Each page is verified against the firmware source.

- [Plugins](/power/plugins/) - install and run sandboxed WASM plugins.
- [Serial console](/power/serial-console/) - the USB CDC command shell.
- [Expert menu](/power/expert-menu/) - hardware info, module control, backup, duress PIN, secure-element cache maintenance and bootloader entry.
- [Languages](/power/languages/) - how the English fallback and the on-device language overlays work, and how to add a language.
- [File storage & NVS tools](/power/storage-tools/) - the vFAT file explorer (main-menu Files and Expert System Files) and the NVS browser/editor.
- [Companion tools](/power/companion-tools/) - the host-side scripts in `tools/`.
